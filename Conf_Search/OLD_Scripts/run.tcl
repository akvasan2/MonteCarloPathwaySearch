...
############  0. psf, pdb files with protein and antibiotic ###########
##################################################################################
...
for {set t_len 0} {$t_len<$frame_number} {incr $t_len 100000} {
	if {[expr $t_len+100000]>$frame_number} {
		animate write dcd dcds/start_${it}.dcd beg $t_len end -1  waitfor all
	} else {
	animate write dcd dcds/start_${it}.dcd beg $t_len end [expr $t_len+100000] waitfor all
	}
} 
